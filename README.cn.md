...
| 文件 | 用途 |
|---|---|
| `holepunch_server.py` | Rendezvous server。记录每个 client 的 public endpoint 并按固定间隔（默认 3s）广播 peer list。两种 client 策略都使用它。 |
...
**服务端广播模型：** peer list 按固定定时器广播（默认每 3s，通过 `--broadcast-interval` 控制），**不是**每收到一个 HELLO 就广播。两次广播之间的 keep-alive HELLO 只更新客户端的 last-seen 时间戳，不会触发额外广播 —— 这样无论多少客户端在 keep-alive，server→client 流量都保持稳定可预测。
...
| `--punch-interval` | 0.5 | Puncher 到对端的 keep-alive packet 间隔（秒）。越低 = mapping 更稳定，流量更多。 |
| `--hello-interval` | 3.0 | Client 到 server 的 HELLO keep-alive 间隔（秒）。应 ≥ server 的 `--broadcast-interval`。 |
| Server `--broadcast-interval` | 3.0 | Peer list 广播间隔（秒）。两次广播之间的 keep-alive 静默接受。 |
| Server `--timeout` | 30 | 多少秒无流量后把 client 从 peer list 中 prune。 |
...
| QoS 友好（低流量） | server：`--broadcast-interval 5`；client：`--hello-interval 5 --scan-rate 50` |
